...
if __name__ == "__main__":
    ...
    processes = 4
    environments_Bline = make_envs_as_vec(seed=0, processes=processes, gamma=0.95, env=cyborg)
    agents = {'Red': RedMeanderAgent}
    path = str(inspect.getfile(CybORG))
    path = path[:-10] + '/Shared/Scenarios/Scenario1b.yaml'
    cyborg = CybORG(path, 'sim', agents)
    environments_RedM = make_envs_as_vec(seed=0, processes=processes, gamma=0.95, env=cyborg)
    ...
    action_space = environments_Bline.action_space.n
    obs_space    = environments_Bline.observation_space.shape[0]
    ...
    agent = AgentA2CSSD(rnd=config.rnd, action_space=action_space, processes=processes, input_space=obs_space)
    print('Agent Created...')
    observation = environments_Bline.reset()
    ...
    for ep in range(config.pre_obs_norm):
        obs = environments_Bline.reset()
        for step in range(config.episode_length):
            actions = np.array([np.random.randint(0, action_space) for i in range(processes)])
            obs, _, _, _ = environments_Bline.step(actions)
    ...
    while episode < int(config.training_length/2):
        ep_loss = []
        episode_rewards = []
        episode_disc_rewards = 0
        observations = environments_Bline.reset()
        ...
        for step in range(0, config.episode_length):
            ...
            observation, reward, done, infos = environments_Bline.step(continuous_action)
    ...
    environments_Bline.close()
    for ep in range(config.pre_obs_norm):
        obs = environments_RedM.reset()
        for step in range(config.episode_length):
            actions = np.array([np.random.randint(0, action_space) for i in range(processes)])
            obs, _, _, _ = environments_RedM.step(actions)
    ...
    for i in range(500):
        for step in range(config.episode_length):
            ...
            for i in range(processes):
                if prob_take_rand > random.uniform(0,1):
                    continuous_action[i] = np.random.randint(0, action_space)

            observation, reward, done, infos = environments_RedM.step(continuous_action)

            masks = torch.FloatTensor([[0.0] if done_ else [1.0] for done_ in done])
            bad_masks = torch.FloatTensor([[0.0] if 'bad_transition' in info.keys() else [1.0] for info in infos])
            rollouts.insert(observation, rnn_states, continuous_action, action_log_prob, value, total_reward, masks,
                            bad_masks)
        with torch.no_grad():
            next_value = agent.actor_critic.get_value(rollouts.observations[-1],
                                                      rollouts.rnn_states[-1],
        ...
        value_loss, action_loss, dist_entropy = agent.update(rollouts)
        rollouts.after_update()
        prob_take_rand = prob_take_rand - (prob_take_rand/500)
    ...
    while episode < int(config.training_length/2):
        ...
        episode_rewards = []
        episode_disc_rewards = 0
        observations = environments_RedM.reset()
