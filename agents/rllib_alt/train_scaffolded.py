...
class CybORGScaffoldedAgent(gym.Env):
    ...
    def step(self, action=None):
        ...
        if isinstance(self.env.get_last_action('Red'), ExploitRemoteService):
            print(self.env.get_last_action('Red'))
        #print(type(self.env.get_last_action('Red')))
        return result
...
if __name__ == "__main__":
    ...
    ModelCatalog.register_custom_model("CybORG_PPO_Scaffolded_Model", CustomModel)

    config = {
        "env": CybORGScaffoldedAgent,
        "env_config": {
            "null": 0,
        },
        ...
        "model": {
            "custom_model": "CybORG_PPO_Scaffolded_Model",
    ...
    analysis = tune.run(ppo.PPOTrainer, # Algo to use - alt: ppo.PPOTrainer, impala.ImpalaTrainer, dqn.ApexTrainer
                        config=config, 
                        local_dir=log_dir,
                        stop=stop,
