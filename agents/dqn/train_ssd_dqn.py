...
from dqn_ssd_agent import SSDAgent
...
if __name__ == "__main__":
    ...
    #state space is 1, 2823, 2823, 2823, 2823
    state_space_decompositions = [1, 2823, 2823, 2823, 2823]
    slices = [[0], [1, 2824], [2824, 5647], [5647, 8470], [8470, 11293]]
    ssd_agent = SSDAgent(action_space=action_space, gamma=config.gamma,
                  epsilon=config.epsilon, lr=config.learning_rate,
                         batch_size=config.batch_size, decomp=state_space_decompositions, slices=slices)
    print('Agent Created...')
    ...
    while episode < config.training_length:
        ...
        for step in range(0, config.episode_length):
            ...
            #ssd_observations = [observation[0], observation[1:2824], observation[2824:5647], observation[5647:8470], observation[8470:]]
            action = ssd_agent.get_action(observation)


            next_observation, reward, done, infos = environment.step(action)
            total_reward = reward
            loss = ssd_agent.dqn.train_q_network(minibatch, priority=config.priority)
            xp_buffer.add_transition([observation, action, reward, next_observation, done])

            episode_rewards.append(reward)
            observation = next_observation
            ep_loss.append(loss)
        ssd_agent.update_epsilon()
        num_eps_this_form += 1
