...
class DQN:
    ...
    def __init__(self, input_dimension=1, output_dimension=24, gamma=0.9, lr=0.0001,
                 batch_size=32, rnd_params=None, attention=None):
    ...
    def train_q_network(self, minibatch, priority, rnd, entropy=None, slice=None):
        # Set all the gradients stored in the optimiser to zero.
        self.optimiser.zero_grad()
        # Calculate the loss for this transition.
        loss = self._calculate_loss(minibatch, priority, slice)
        if priority:
    ...
    def _calculate_loss(self, minibatch, priority, slice):
        if priority:
        ...
        if slice is not None:
            if len(slice) == 2:
                next_state_tensor = next_state_tensor[:, slice[0]:slice[1]]
                state_tensor = state_tensor[:, slice[0]:slice[1]]
            elif len(slice) == 1:
                next_state_tensor = next_state_tensor[:, slice[0]].unsqueeze(-1)
                state_tensor = state_tensor[:, slice[0]].unsqueeze(-1)
...
class AverageDQN:
    ...
    # Function to train the Q-network
    def train_q_network(self, minibatch, priority, rnd, slice=None):
        # Set all the gradients stored in the optimiser to zero.
        self.optimiser.zero_grad()
        # Calculate the loss for this transition.
        loss = self._calculate_loss(minibatch, priority, slice)
    ...
    def _calculate_loss(self, minibatch, priority, slice):
        ...
        if slice is not None:
            if len(slice) == 2:
                ...
                next_state_tensor = next_state_tensor[slice[0]]
